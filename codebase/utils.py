...
def log_normal(x, m, v):
    ...
    # the last dimension
    ################################################################################
    ln = torch.distributions.normal.Normal(m, v.sqrt())
    log_prob = ln.log_prob(x).sum(-1)
    ################################################################################
    # End of code modification
    ################################################################################
    return log_prob
...
def log_normal_mixture(z, m, v):
    ...
    ################################################################################
    # TODO: Modify/complete the code here
    # Compute the uniformly-weighted mixture of Gaussians density for each samplez
    # in the batch
    ################################################################################
    n_ = torch.distributions.normal.Normal(m, v.sqrt())
    log_prob_raw = n_.log_prob(z.unsqueeze(1))
    #
    # Get the probability of the mixture model.
    log_prob_mix = log_mean_exp(log_prob_raw, 1)
    #
    # Get the probability of the sample.
    log_prob = log_prob_mix.sum(-1)
    ################################################################################
    # End of code modification
    ################################################################################
    return log_prob
...
def get_mnist_data(device, use_test_subset=True):
    ...
    yl = torch.eye(10, dtype=torch.long)[yl.cpu()].to(device)
